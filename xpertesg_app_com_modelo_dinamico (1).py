...
if st.session_state.usuario:
    ...
    import joblib
    st.title("📊 Modelo ESG - Análise de Propensão")

    # Carrega modelos e transformador
    model1 = joblib.load("modelo_logreg.pkl")
    model2 = joblib.load("modelo_rf.pkl")
    preproc = joblib.load("preprocessor.pkl")

    # Seleciona cliente
    st.subheader("Selecione um cliente")
    nome_cliente = st.selectbox("Nome do cliente:", df["nome"].unique())
    cliente = df[df["nome"] == nome_cliente].copy()

    # Aplica pré-processamento
    X_cliente = cliente.drop(columns=["nome"])
    X_proc = preproc.transform(X_cliente)

    # Etapa 1: Logistic Regression
    prob1 = model1.predict_proba(X_proc)[:, 1]
    pred1 = prob1 >= 0.15

    if pred1[0]:
        pred_final = model2.predict(X_proc)[0]
        label = "✅ ESG" if pred_final == 1 else "❌ Não ESG"
    else:
        pred_final = 0
        label = "❌ Não ESG"

    # Mostra resultados
    st.metric("Propensão ESG (etapa 1)", f"{prob1[0]:.2f}")
    st.metric("Classificação Final", label)
